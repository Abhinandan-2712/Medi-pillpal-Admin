    ...
    try {
      ...
      if (res?.data?.success) {
        ...
        if (message === "Admin not found") {
          toast.error("You are not an admin, please check your email.", {
            id: "error",
          });
        } else if (message === "Password mismatch") {
          toast.error("Password mismatch, please try again.", { id: "error" });
        } else {
          // Optional: handle other error messages
          toast.error(message || "Something went wrong.", { id: "error" });
        }
      }
    } catch (err) {
